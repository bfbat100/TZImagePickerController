...
- (instancetype)init {
    self = [super init];
    if (self) {
        self = [self initWithMaxImagesCount:9 isNeedSizeControl:false videoMaxDuration:120 delegate:nil];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    [TZImageManager manager].showHud = ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [self showProgressHUD];
        });
    };
    [TZImageManager manager].hideHud = ^{
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount isNeedSizeControl:(BOOL)isNeedSizeControl videoMaxDuration:(float)videoMaxDuration delegate:(id<TZImagePickerControllerDelegate>)delegate{
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 isNeedSizeControl:isNeedSizeControl  videoMaxDuration:(float)videoMaxDuration delegate:delegate pushPhotoPickerVc:YES];
}
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber isNeedSizeControl:(BOOL)isNeedSizeControl videoMaxDuration:(float)videoMaxDuration delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    _isNeedSizeControl = isNeedSizeControl;
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ...
        photoPickerVc.isNeedSizeControl = _isNeedSizeControl;
        photoPickerVc.videoMaxDuration = _videoMaxDuration;
...
//    CGFloat progressHUDY = CGRectGetMaxY(self.navigationBar.frame);
    //设置全屏遮罩
    _progressHUD.frame = CGRectMake(0, 0, self.view.tz_width, self.view.tz_height);
    _HUDContainer.frame = CGRectMake((self.view.tz_width - 120) / 2, (_progressHUD.tz_height - 90) / 2, 120, 90);
