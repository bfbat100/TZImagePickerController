...
@interface TZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate,HXVideoEditViewControllerDelegate,
...
- (void)configBottomToolBar {
    ...
    if (_tzImagePickerVc.allowEditPhoto) {
        _editButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _editButton.titleLabel.font = [UIFont systemFontOfSize:16];
        [_editButton addTarget:self action:@selector(editButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [_editButton setTitle:@"编辑" forState:UIControlStateNormal];
        [_editButton setTitleColor:_tzImagePickerVc.oKButtonTitleColorNormal forState:UIControlStateNormal];
//        [_editButton setImage:[UIImage tz_imageNamedFromMyBundle:@"video_edit"] forState:UIControlStateNormal];
    }
    ...
    [_toolBar addSubview:_editButton]; //编辑图片
...
- (void)viewDidLayoutSubviews {
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        if (_tzImagePickerVc.allowEditPhoto) { //调整按钮位置
    ...
    _editButton.frame = CGRectMake(12, 13, 40, 18);
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (!_tzImagePickerVc.allowPickingImage && _tzImagePickerVc.allowPickingVideo ){
                [_tzImagePickerVc showAlertWithTitle:@"你最多只能选择1个视频"];
            ...
            [TZImageManager judgeAssetBigger:model isNeedSizeControl:YES  result:^(BOOL isFail, NSString *errorMsg) {
                //                           NSLog(@"22222=====%d",[TZImageManager manager].videoNum);
                if(isFail){
...
- (void)photoEditingController:(HX_PhotoEditViewController *)photoEditingVC didFinishPhotoEdit:(HXPhotoEdit *)photoEdit photoModel:(nonnull HXPhotoModel *)photoModel {
    ...
    [photoModel getImageWithSuccess:^(UIImage * _Nullable image, HXPhotoModel * _Nullable model, NSDictionary * _Nullable info) {
        ...
        [cell.previewView.imageView setImage:image];
...
//                    NSLog(@"======%@", asset);
                    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
//                    NSLog(@"%@********%ld***%@",_tzImagePickerVc.selectedModels,self.currentIndex,self->_indexLabel.text);
                    TZAssetModel *model = [TZAssetModel modelWithAsset:asset type:TZAssetModelMediaTypePhoto];
                    model.isSelected = YES;
                    
                    NSLog(@"======%@", _tzImagePickerVc.selectedAssetIds);
                    [_tzImagePickerVc.selectedModels replaceObjectAtIndex:[self->_indexLabel.text  integerValue]-1 withObject:model];
                    [self.models  replaceObjectAtIndex:self.currentIndex withObject:model];
                } else if (error) {
...
//    NSLog(@"%@-----%@",self.models,self.photos);
    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
//    NSLog(@"%@********%ld***",_tzImagePickerVc.selectedModels,self.currentIndex);
    ...
    self.photoManager.configuration.maxVideoClippingTime = 30;

//    NSString *path = [[NSBundle mainBundle] pathForResource:@"LocalSampleVideo" ofType:@"mp4"];
//    NSURL *url = [NSURL fileURLWithPath:path];
//    HXPhotoModel *model = [HXPhotoModel photoModelWithVideoURL:url];
        
    TZAssetModel *assetModel = _tzImagePickerVc.selectedModels[0];
    HXPhotoModel *model = [HXPhotoModel photoModelWithPHAsset:assetModel.asset];
...
//    HXPhotoModel *model = [HXPhotoModel photoModelWithImage:[UIImage imageNamed:@"wx_bg_image"]];
