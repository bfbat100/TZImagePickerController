...
//    [self.view addSubview:_cropVideoDurationLabel];  //隐藏截取时间
...
//            [self.imagePickerVc.pickerDelegate imagePickerController:self.imagePickerVc didFinishPickingAndEditingVideo:coverImage outputPath:_outputPath error:nil];
            AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:[NSURL fileURLWithPath:_outputPath]  options:nil];
            [[TZImageManager manager] compressVideoWithVideoUrl:asset.URL withBiteRate:@(2000 * 1024) withFrameRate:@(20) withVideoWidth:@(1080) withVideoHeight:@(1920)  compressComplete:^(id responseObjc) {
                                NSLog(@"压缩成功%@",responseObjc);
            [self.imagePickerVc.pickerDelegate imagePickerController:self.imagePickerVc didFinishPickingAndEditingVideo:coverImage outputPath:responseObjc[@"urlStr"] error:nil];
                dispatch_async(dispatch_get_main_queue(), ^{
