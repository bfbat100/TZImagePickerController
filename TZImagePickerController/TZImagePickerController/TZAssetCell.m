...
//- (void)setIsShowModalView:(BOOL)isShowModalView {
//    _isShowModalView = isShowModalView;
//    if (isShowModalView) {
            ...
            [TZImageManager judgeAssetBigger:self.model isNeedSizeControl:YES result:^(BOOL isFail, NSString *errorMsg) {
                if (isFail) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [UIView showOscillatoryAnimationWithLayer:self->_selectImageView.layer type:TZOscillatoryAnimationToBigger];
                    });
                    // 用户选中了该图片，提前获取一下大图
                    [self requestBigImage];
                }
...
- (UIView *)modalView {
    if (_modalView == nil) {
        ...
        modalView.backgroundColor = [UIColor grayColor];
        modalView.alpha = 0.7;
        [self.contentView addSubview:modalView];
...
- (void)layoutSubviews {
    ...
    _modalView.frame =  CGRectMake(0, 0, self.tz_width, self.tz_height);
    ...
    [self.contentView bringSubviewToFront:_modalView];
