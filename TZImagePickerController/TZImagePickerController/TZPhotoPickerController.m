...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for (int i = 0; i <  self->_models.count; i++) {
            ...
            if (model.type == TZAssetModelMediaTypeVideo) { // 视频
                [[PHImageManager defaultManager] requestAVAssetForVideo:model.asset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
                    if ([asset isKindOfClass:[AVURLAsset class]]) {
                        ...
                        if ([size floatValue]/(1024.0*1024.0) > 30.0 && self.isNeedSizeControl) { //大于30M
                            model.isBiggerThirty = YES;
                        }else {
                            model.isBiggerThirty = NO;
                        }
                        [tmp addObject:model];
                    }}];
            }
                ...
                [imageManager requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                    ...
                    if (length / (1024.0*1024.0) > 30.0 && self.isNeedSizeControl) { //大于30M
                        model.isBiggerThirty = YES;
                    }else {
                        model.isBiggerThirty = NO;
                    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSLog(@"当前模式%d",self.isNeedSizeControl);
    if (!_models) {
...
//    if (model.isBiggerThirty) {  //大于30M不可点击
//        return;
//    }
